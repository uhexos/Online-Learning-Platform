  ...
  render() {
    ...
    const printLesson = () => {
      ...
      return this.props.lessons.map((lesson, index) => (
        // assign each nav item a unique id. this is used to select the correct menu item to click when mounting a page.
        <NavItem key={index} id={`nav-item-${index}`} onClick={()=>this.props.getLesson(lesson.id)}>
